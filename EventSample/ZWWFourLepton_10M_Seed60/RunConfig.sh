...
# Create Gridpack
/work/home/ruttho/binary/MG5_aMC_v2_9_24/bin/mg5_aMC eeToZWW_FourLeptons_1M_Seed60_mg5Card.dat
# Clean Directory
rm -rf eeToZWW_FourLeptons
rm -rf *.lhe
rm -rf *.hepmc
rm -rf *.root
# Move grippack to working directory and unpack
mv eeToZWW_FourLeptons/formal01_gridpack.tar.gz .
...
for i in {1..10}; do
    ...
    mv Delphes_job${i}.root ../ZWWFourLepton_1M_Part${i}_Seed${SEED}.root
    mv events.lhe.gz ../ZWWFourLepton_1M_Part${i}_Seed${SEED}_unweighted_events.lhe.gz
    mv tag_1_pythia8_events.hepmc ../ZWWFourLepton_1M_Part${i}_Seed${SEED}_pythia8_events.hepmc
    cd ..
    ) &
done
wait
